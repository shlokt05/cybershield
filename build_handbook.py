...
handbook_code = '''export interface HandbookSection {
  sectionTitle: string;
  subheading?: string;
  content: string;
        ...
        sections: [
          {
            ...
            technicalDetails: [
              'Client-Side JS Bypass: Browser validation attributes (pattern, required, maxlength) are easily stripped using intercepting proxies.',
              'Server-Side Schema Validation: Backend APIs must validate payload data against strict schemas (Zod, Joi) enforcing type safety and string boundaries.',
              'Least Privilege & Data Isolation: Database connection pools must operate under restricted system roles.'
            ],
            keyPoints: [
          ...
          {
            sectionTitle: '1.4 HTTP Request Smuggling & Protocol Parsing Vulnerabilities',
            subheading: 'Content-Length vs. Transfer-Encoding (CL.TE / TE.CL) Mismatches in Reverse Proxies',
            content: 'HTTP Request Smuggling arises when front-end reverse proxies (e.g., NGINX, HAProxy) and back-end application servers parse HTTP request boundaries differently. When a request contains both a Content-Length (CL) header and a Transfer-Encoding: chunked (TE) header, inconsistent processing causes the back-end server to treat part of the request as the start of a subsequent user request. Attackers exploit this to bypass front-end WAF controls, poison web caches, and hijack valid user session cookies.',
            technicalDetails: [
              'CL.TE Vector: Front-end uses Content-Length, while Back-end uses Transfer-Encoding.',
              'TE.CL Vector: Front-end uses Transfer-Encoding, while Back-end uses Content-Length.',
              'Remediation: Upgrade to HTTP/2 end-to-end or configure proxies to reject requests containing ambiguous header combinations.'
            ],
            keyPoints: [
              'HTTP Request Smuggling exploits boundary parsing differences between front-end proxies and back-end application servers.',
              'Never allow requests with both Content-Length and Transfer-Encoding headers to pass through reverse proxies.',
              'HTTP/2 natively eliminates request smuggling by replacing text-based delimiting with binary framing.'
            ]
          },
          {
            sectionTitle: '1.5 Data Serialization Vulnerabilities: XXE & Deserialization Gadgets',
            subheading: 'XML External Entity (XXE) Injection, PyYAML Unsafe Load & Java ObjectInputStream Exploit Mechanics',
            content: 'Unsafe data deserialization occurs when an application reconstructs binary or structured objects (XML, YAML, Java serialized objects) supplied by an untrusted client. XML External Entity (XXE) injection occurs when weakly configured XML parsers process external entity references, allowing attackers to exfiltrate local files (/etc/passwd) or trigger internal SSRF requests. In Python, calling yaml.load() on untrusted strings executes arbitrary Python code unless yaml.safe_load() is used.',
            codeSnippet: {
              language: 'python',
              code: '# VULNERABLE YAML DESERIALIZATION:\\nimport yaml\\ndata = yaml.load(user_input) # VULNERABLE! Allows remote code execution\\n\\n# SECURE SAFE DESERIALIZATION:\\ndata = yaml.safe_load(user_input) # SECURE! Restricts parsing to safe primitives',
              note: 'Always use yaml.safe_load() or JSON serialization instead of unsafe language object deserialization.'
            },
            keyPoints: [
              'Disable DTD external entity resolution (disallow-doctype-decl) in XML parsers to prevent XXE.',
              'Never use native language object deserialization (Java ObjectInputStream, Python pickle, PyYAML load) on untrusted inputs.',
              'Prefer strict data-only formats like JSON or Protocol Buffers with predefined schemas.'
            ]
          },
          {
            sectionTitle: '1.6 Client Storage Security: LocalStorage vs. HttpOnly Cookie Isolation',
            subheading: 'XSS Attack Surfaces, Web Storage Access Risk & Token Management Patterns',
            content: 'Web applications store authentication state using client-side mechanisms such as LocalStorage, SessionStorage, or HttpOnly Cookies. Storing JWTs or sensitive bearer tokens inside LocalStorage or SessionStorage creates a critical security flaw: any XSS vulnerability anywhere on the application domain can read window.localStorage and exfiltrate credentials instantly. HttpOnly cookies isolate tokens from browser JavaScript execution entirely, mitigating XSS session theft.',
            keyPoints: [
              'LocalStorage and SessionStorage are fully accessible to browser JavaScript, making them vulnerable to XSS token theft.',
              'HttpOnly cookies hide tokens from document.cookie, protecting credentials from client-side script inspection.',
              'Combine HttpOnly, Secure, SameSite=Strict, and anti-CSRF tokens for comprehensive token isolation.'
            ]
          },
          {
            sectionTitle: '1.7 Server-Side Input Validation Architecture & Whitelist Schemas',
            subheading: 'Production Zod/Joi Schema Implementations, Regex Bound Checks & Numeric Range Constraints',
            content: 'Input validation is the primary line of defense against malformed or malicious data entering application workflows. Robust validation relies on Whitelisting (defining exactly what input format is allowed) rather than Blacklisting (attempting to block known malicious strings). Using schema validation libraries like Zod or Joi enforces strict data types, string length bounds, numeric range limits, and regex pattern matching before data reaches business logic or database queries.',
            codeSnippet: {
              language: 'typescript',
              code: 'import { z } from "zod";\\n\\n// Strict Whitelist User Registration Schema:\\nexport const UserRegisterSchema = z.object({\\n  username: z.string().min(3).max(30).regex(/^[a-zA-Z0-9_-]+$/),\\n  email: z.string().email().max(100),\\n  age: z.number().int().min(18).max(120),\\n  role: z.enum(["user", "auditor"])\\n});',
              note: 'Zod schemas validate type, length, pattern, and allowed values, dropping extra unexpected parameters.'
            },
            keyPoints: [
              'Whitelist input validation strictly defines accepted characters, lengths, ranges, and structures.',
              'Blacklisting fails because attackers continuously discover bypasses and encoding tricks.',
              'Validate all input parameters at API controller entry points before passing data to service layers.'
            ]
          },
          {
            sectionTitle: '1.8 Modern API Gateway Security & Token Bucket Rate-Limiting Architecture',
            subheading: 'Redis Rate Limiting, DDoS Mitigation, Cloudflare WAF Rules & Signature Authentication',
            content: 'API Gateways serve as the front line of defense for microservice architectures, managing incoming traffic, enforcing authentication, and mitigating brute-force attacks. Utilizing the Token Bucket algorithm backed by distributed Redis clusters, gateways enforce per-IP and per-user rate limits (e.g., maximum 100 requests per minute). Furthermore, API gateways inspect HTTP headers, drop suspicious payloads, and verify HMAC API signatures before forwarding requests to internal services.',
            keyPoints: [
              'Token Bucket algorithms backed by Redis enforce distributed rate limiting to defeat brute-force attacks.',
              'API Gateways offload authentication, TLS termination, and WAF inspection from microservices.',
              'HMAC API request signatures verify message integrity and request timestamp validity.'
            ]
          }
        ]
      },
...
with open("src/lib/handbookData.ts", "w", encoding="utf-8") as f:
    f.write(handbook_code)

print("Updated handbookData.ts successfully!")
