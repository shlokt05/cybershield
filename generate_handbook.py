...
# Helper to generate chapters for a module
def create_section(title, sub, content, code=None, lang="javascript", note=None, takeaways=None):
    if takeaways is None:
        takeaways = [
            "Always enforce strict server-side input validation whitelist schemas.",
            "Apply principle of least privilege and strict boundary controls.",
            "Incorporate automated security audits in the CI/CD pipeline."
        ]
    sec = {
        "sectionTitle": title,
        "subheading": sub,
    ...
    if code:
        sec["codeSnippet"] = {
            "language": lang,
            "code": code,
            "note": note or "Follow production security guidelines."
        }
    return sec

def build_web_sec_chapters():
    return [
        {
            "chapterNumber": 1,
            ...
            "sections": [
                create_section(
                    "1.1 Trust Boundaries & Untrusted Data Ingestion Mechanics",
                    "Detailed Notes on Client Validation Bypasses, Proxy Manipulation & Backend Zod/Joi Schemas",
                ...
                create_section(
                    "1.2 HTTP State Management & Cookie Security Notes",
                    "Detailed Notes on Session Token Hijacking, Fixation & Modern SameSite Directives",
                ...
                create_section(
                    "1.4 HTTP Request Smuggling & Protocol Boundary Desynchronization",
                    "Front-End Proxy vs Back-End Server Transfer-Encoding & Content-Length Parsing Discrepancies",
                    "HTTP Request Smuggling occurs when front-end reverse proxies (NGINX, HAProxy) and back-end application servers parse HTTP request boundaries differently. When a request contains both Content-Length (CL) and Transfer-Encoding: chunked (TE) headers, desynchronization allows attackers to smuggle hidden HTTP requests into the back-end socket pool. Attackers exploit this to bypass WAF rules, steal session cookies, and poison web cache caches.",
                ...
                create_section(
                    "1.6 Client Storage Isolation: LocalStorage vs. HttpOnly Cookie Security Models",
                    "XSS Session Exfiltration Hazards, Web Storage API Access & Cookie Scope Attributes",
                ...
                create_section(
                    "1.7 Server-Side Input Validation Architecture & Production Whitelist Schemas",
                    "Zod, Joi & Express-Validator Implementations, Regex Boundary Checks & Type Constraints",
                    "Server-side input validation requires Whitelisting (defining exact allowed structures and patterns) rather than Blacklisting. Using TypeScript Zod schemas at API controller entry points ensures incoming JSON payloads match strict types, string lengths, and regex patterns before hitting internal services.",
        ...
        },
        {
            ...
            "sections": [
                ...
                create_section(
                    "2.4 Database Hardening, Least Privilege & Network Isolation",
                    "Restricting Database User Permissions, Disabling xp_cmdshell & Connection Encryption",
        ...
        },
        {
            ...
            "sections": [
                create_section(
                    "3.1 XSS Vectors & Client Sanitization Notes",
                    "Stored, Reflected, DOM-Based Execution & DOMPurify Engine Mechanics",
                    "Cross-Site Scripting allows attackers to execute arbitrary client-side JavaScript in a victim browser session. Stored XSS persists in database records, Reflected XSS reflects off URL parameters, and DOM XSS executes within client-side script sinks like element.innerHTML.",
                ...
                create_section(
                    "3.3 Content Security Policy (CSP) Architecture & Nonce Enforcement",
                    "script-src Directives, Strict Nonces, Hash Pinning & Report-Only Audits",
                    "Content Security Policy (CSP) is an HTTP response header that mandates authorized origins for scripts, styles, images, and child frames. A strict CSP neutralizes XSS by instructing the browser to reject inline script tags (<script>alert(1)</script>) and unauthorized external scripts. Implementing script-src 'nonce-rAnd0m123' ensures that only scripts possessing a cryptographically random, per-request nonce will execute.",
                ...
                create_section(
                    "3.4 Context-Aware Output Encoding Rules",
                    "HTML Body, Attribute, JavaScript, CSS & URL Encoding Context Boundaries",
                    "Defending against XSS requires context-aware output encoding. Plain HTML entity encoding (&lt; &gt; &amp; &quot;) protects HTML body context, but fails inside JavaScript blocks, attribute values, or CSS declarations. For example, placing user input inside an HTML attribute (<input value=\"UNTRUSTED\">) requires encoding quotes and spaces, whereas placing input inside a JS variable (let data = \"UNTRUSTED\";) requires Unicode JS escaping (\\u0027).",
                    takeaways=[
        ...
        },
        {
            ...
            "sections": [
                create_section(
                    "4.1 Response Header Architecture Notes",
                    "HSTS Max-Age, CSP Directives & Clickjacking Defense",
                    "Content Security Policy (CSP) headers dictate authorized script origins, while HSTS (Strict-Transport-Security: max-age=31536000) forces browsers to use HTTPS for one full year. X-Frame-Options: DENY prevents clickjacking framing attacks.",
                    takeaways=[
                ...
                create_section(
                    "4.2 Cross-Origin Resource Sharing (CORS) Security & Credential Restrictions",
                    "Access-Control-Allow-Origin, Wildcard Hazards & Preflight Validation",
                    "Cross-Origin Resource Sharing (CORS) is a browser security mechanism that allows web applications running at one domain to access resources at a different origin. Misconfigured CORS headers represent a major security risk. Specifically, combining Access-Control-Allow-Origin: * with Access-Control-Allow-Credentials: true is forbidden by browser specifications, but dynamic origin reflection (echoing Origin header back to client) allows attackers to read private user API responses.",
                    takeaways=[
        ...
        },
        {
            ...
            "sections": [
                create_section(
                    "5.1 Enterprise Code Audit Checklist & Penetration Testing Workflow",
                    "Methodology for Reviewing Codebases, Dependency Trees & API Endpoints",
                    "Performing an enterprise application security code audit requires a systematic approach. Reviewers inspect authentication flows, verify input validation schemas at controller boundaries, audit database queries for parameterization, and check HTTP response headers. Additionally, auditing third-party npm/PyPI dependencies using automated scanners (npm audit, Snyk) ensures known CVE vulnerabilities are patched.",
                    takeaways=[
                ...
                create_section(
                    "5.2 Comprehensive Exam Answer Key Notes",
                    "Question-by-Question Solution Explanations for Questions 1 to 20",
                    "Verified solutions and full explanations for all 20 Assessment Questions in the Web Security Module:",
                    takeaways=[
...
# Execute script to write handbookData.ts
import json

web_chaps = build_web_sec_chapters()

ts_content = f'''export interface HandbookSection {{
  sectionTitle: string;
  subheading?: string;
  content: string;
...
export const MODULE_HANDBOOKS: Record<string, ModuleHandbook> = {{
  'web-security': {{
    ...
    chapters: {json.dumps(web_chaps, indent=6)},
    appendixExamAnswers: [
...
with open("src/lib/handbookData.ts", "w", encoding="utf-8") as f:
    f.write(ts_content)

print("Generated comprehensive handbookData.ts with 8 detailed sections per chapter!")
